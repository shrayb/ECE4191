...
front_left_sonic = Ultrasonic(echo_pin=front_left_sonic_echo, trig_pin=front_left_sonic_trig, x_offset=0.08512, y_offset=0.05868, theta=22.5*math.pi/180, reading_index=0, maximum_read_distance=0.3)
front_right_sonic = Ultrasonic(echo_pin=front_right_sonic_echo, trig_pin=front_right_sonic_trig, x_offset=0.08512, y_offset=-0.05868, theta=-22.5*math.pi/180, reading_index=1, maximum_read_distance=0.3)
middle_sonic = Ultrasonic(echo_pin=middle_sonic_echo, trig_pin=middle_sonic_trig, x_offset=0.08478, y_offset=0, theta=0, reading_index=2, maximum_read_distance=0.3)
left_sonic = Ultrasonic(echo_pin=left_sonic_echo, trig_pin=left_sonic_trig, x_offset=-0.0535, y_offset=0.11172, theta=math.pi/2, reading_index=3, maximum_read_distance=0.2)
right_sonic = Ultrasonic(echo_pin=right_sonic_echo, trig_pin=right_sonic_trig, x_offset=-0.535, y_offset=-0.11172, theta=-math.pi/2, reading_index=4, maximum_read_distance=0.2)

limit_switch = LimitSwitch(distance=0.105, switch_pin=limit_switch_pin)
...
def mainloop():
    try:
        while True:
            distance = robot.package_ultrasonic.measure_dist()
            print(distance)
