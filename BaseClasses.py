...
class Package:
    ...
    def identify_destinations(self):
        if self.ID == 0:
            self.destination_pose = Pose(x=0.23, y=0.97, theta=math.pi/2)
        if self.ID == 1:
            self.destination_pose = Pose(x=0.6, y=0.97, theta=math.pi/2)
        if self.ID == 2:
            self.destination_pose = Pose(x=0.97, y=0.97, theta=math.pi/2)
