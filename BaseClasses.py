...
class Package:
    ...
    def identify_destinations(self):
        if self.colour == "red":
            self.destination_pose = Pose(x=0.2, y=12, theta=math.pi/2)
            self.return_destination = Pose(x=0.3, y=0.4)
        if self.colour == "green":
            self.destination_pose = Pose(x=0.6, y=0.9, theta=math.pi/2)
            self.return_destination = Pose(x=0.3, y=0.4)
        if self.colour == "blue":
            self.destination_pose = Pose(x=1, y=0.9, theta=math.pi/2)
            self.return_destination = Pose(x=0.3, y=0.4)
...
class ColourSensor:
    def __init__(self, s2=None, s3=None, signal=None):
        ...
        self.ranges = [[27439.399152705606, 19675.80216518888, 24400.394143267447], [22112.20804285669, 24593.719507634007, 29048.575834942832], [19280.75535718807, 19464.75947248711, 20133.35011071063]]  # R G B
...
class Ultrasonic:
    ...
    def measure_dist(self):
        ...
        pulse_duration = pulse_end - pulse_start - 0.001
