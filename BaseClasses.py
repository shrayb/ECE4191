...
class Package:
    ...
    def identify_destinations(self):
        if self.colour == "red":
            self.destination_pose = Pose(x=0.2, y=1.05, theta=math.pi/2)
        if self.colour == "green":
            self.destination_pose = Pose(x=0.6, y=1.05, theta=math.pi/2)
        if self.colour == "blue":
            self.destination_pose = Pose(x=1, y=1.05, theta=math.pi/2)
